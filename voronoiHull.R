...
necessary=c("dismo","SDMTools", "deldir", "rgdal", "pROC", "R2HTML", "png") #list the libraries needed
...
if (model.voronoiHull) {
	...
	if (!is.null(vh)) {	
		...
		vh.proj = predict(vh, current.climate.scenario, ext=opt.ext)   # predict for given climate scenario
		saveModelProjection(vh.proj, "voronoiHull", "current") # save output
		rm(list=c("vh", "vh.proj")); #clean up memory
	} else {
...
if (project.voronoiHull) {
	...
	if (!is.null(voronoiHull.obj)) {
		...
		saveModelProjection(voronoiHull.proj, "voronoiHull", "future") 	# save output
...
if (evaluate.voronoiHull) {
	...
	if (!is.null(voronoiHull.obj)) {
		...
		saveModelEvaluation(voronoiHull.eval, voronoiHull.combined.eval, "voronoiHull")	# save output
