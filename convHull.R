src/org/bccvl/compute/rscripts/convHull.R# set CRAN mirror in case we need to download something
...
if (model.convHull) {
	...
	if (!is.null(ch)) {		
		...
		ch.proj = predict(ch, current.climate.scenario, ext=opt.ext)   # predict for given climate scenario
		saveModelProjection(ch.proj, "convHull", "current") # save output
		rm(list=c("ch", "ch.proj")); #clean up memory
	} else {
...
if (project.convHull) {
	...
	if (!is.null(convHull.obj)) {
		...
		saveModelProjection(convHull.proj, "convHull", "future") 	# save output
...
if (evaluate.convHull) {
	...
	if (!is.null(convHull.obj)) {
		...
		saveModelEvaluation(convHull.eval, convHull.combined.eval, "convHull")	# save output
