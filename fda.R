src/org/bccvl/compute/rscripts/fda.R# set CRAN mirror in case we need to download something
...
if (model.fda) {
	...
	if (!is.null(myBiomodModelOut.fda)) {		
		...
		fda.proj.c = my.BIOMOD_Projection(modeling.output=myBiomodModelOut.fda, new.env=current.climate.scenario, proj.name="current", 
...
if (project.fda) {
	...
	if (!is.null(fda.obj)) {
		...
		fda.proj = my.BIOMOD_Projection(modeling.output=fda.obj, new.env=predictors, proj.name="future", 
			xy.new.env=biomod.xy.new.env, selected.models=biomod.selected.models, binary.meth=biomod.binary.meth, 
			filtered.meth=biomod.filtered.meth, compress=biomod.compress, 
			build.clamping.mask=biomod.build.clamping.mask, silent=opt.biomod.silent, do.stack=opt.biomod.do.stack, 
			keep.in.memory=opt.biomod.keep.in.memory, output.format=opt.biomod.output.format)
...
if (evaluate.fda) {	
	...
	if (!is.null(fda.obj)) {
		...
		saveBIOMODModelEvaluation(fda.loaded.model, fda.obj, "fda") 	# save output
