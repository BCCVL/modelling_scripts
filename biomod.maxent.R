src/org/bccvl/compute/rscripts/biomod.maxent.R# set CRAN mirror in case we need to download something
...
if (model.biomod.maxent) {
	...
	if (!is.null(myBiomodModelOut.biomod.maxent)) {		
		...
		biomod.maxent.proj.c = BIOMOD_Projection(modeling.output=myBiomodModelOut.biomod.maxent, new.env=current.climate.scenario, proj.name=es.name, 
