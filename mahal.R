src/org/bccvl/compute/rscripts/mahal.R# set CRAN mirror in case we need to download something
...
if (model.mahal) {
	if (!all(enviro.data.type=="continuous")) {
		...
		if (!is.null(mm)) {	
			...
			mm.proj = predict(mm, current.climate.scenario, tails=opt.tails, ext=opt.ext)   # predict for given climate scenario
            saveModelProjection(mm.proj, "mahal", "current") # save output
			rm(list=c("mm", "mm.proj")); #clean up memory
		} else {
...
if (project.mahal) {
	...
	if (!is.null(mahal.obj)) {
		...
		saveModelProjection(mahal.proj, "mahal", "future") 	# save output
...
if (evaluate.mahal) {
	...
	if (!is.null(mahal.obj)) {
		...
		saveModelEvaluation(mahal.eval, mahal.combined.eval, "mahal")	# save output
