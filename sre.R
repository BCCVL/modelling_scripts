src/org/bccvl/compute/rscripts/sre.R# set CRAN mirror in case we need to download something
...
if (model.sre) {
	...
	if (!is.null(myBiomodModelOut.sre)) {		
		...
		sre.proj.c = my.BIOMOD_Projection(modeling.output=myBiomodModelOut.sre, new.env=current.climate.scenario, proj.name="current", 
...
if (project.sre) {
	...
	if (!is.null(sre.obj)) {
		...
		sre.proj = my.BIOMOD_Projection(modeling.output=sre.obj, new.env=predictors, proj.name="future", 
			xy.new.env=biomod.xy.new.env, selected.models=biomod.selected.models, binary.meth=biomod.binary.meth, 
			filtered.meth=biomod.filtered.meth, compress=biomod.compress, 
			build.clamping.mask=biomod.build.clamping.mask, silent=opt.biomod.silent, do.stack=opt.biomod.do.stack, 
			keep.in.memory=opt.biomod.keep.in.memory, output.format=opt.biomod.output.format)
...
if (evaluate.sre) {	
	...
	if (!is.null(sre.obj)) {
		...
		saveBIOMODModelEvaluation(sre.loaded.model, sre.obj, "sre") 	# save output
