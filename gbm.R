...
necessary=c("biomod2","SDMTools", "gbm", "rgdal", "pROC", "R2HTML", "png") #list the libraries needed
...
if (model.gbm) {
	...
	if (!is.null(myBiomodModelOut.gbm)) {		
		...
		gbm.proj.c = BIOMOD_Projection(modeling.output=myBiomodModelOut.gbm, new.env=current.climate.scenario, proj.name=es.name, 
