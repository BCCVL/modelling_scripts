...
if (model.domain) {
	if (!all(enviro.data.type=="continuous")) {
		...
		if (!is.null(dm)) {	
            ...
            dm.proj = predict(dm, current.climate.scenario, ext=opt.ext)   # predict for given climate scenario
            saveModelProjection(dm.proj, "domain", "current") # save output
			rm(list=c("dm", "dm.proj")); #clean up memory
		} else {
...
if (project.domain) {
	...
	if (!is.null(domain.obj)) {
		...
		saveModelProjection(domain.proj, "domain", "future") # save output
...
if (evaluate.domain) {
	...
	if (!is.null(domain.obj)) {
		...
		saveModelEvaluation(domain.eval, domain.combined.eval, "domain")	# save output
